 ...
 * background policy, or any code that is not a Composable. Requests made here and requests made by
 * a boundary compose through the same reference counting (FR-019), so the two can be mixed freely.
 *
 * **No operation on this object throws** (FR-021). An unsupported capability, an absent window, and
 * a mechanism that will not install are all reported through [supportLevel] and
 * [protectionFailures] rather than as exceptions — a security library that crashed the host app on
 * an unsupported device would be worse than one that degraded.
 *
 * **Thread-safety**: every member is safe to use from any thread (FR-018). Platform effects are
...
public object ComposeGuard {
     ...
     *
     * **Idempotent, not reference-counted** (US5 scenario 2). Two calls with the same
     * [capabilities] share one claim, and releasing either releases it. This differs from
     * [SecureContent], where each boundary is counted separately — an imperative caller has no
     * composition lifetime to make counting exact, and a policy object acquiring on every
     * navigation would otherwise leak protection permanently.
     *
     * A declarative boundary's claim is unaffected either way: releasing this handle never
     * unprotects a window a composed boundary still wants (FR-019).
     *
     * @param capabilities which preventions to request. Independently requestable (FR-005).
     ...
     *
     * Evaluated at call time, never cached (FR-020b). The same call can return
     * [SupportLevel.Supported] and later [SupportLevel.Unsupported] within one session — most
     * notably on Android, where activating screenshot prevention precludes [Capability.ScreenshotEvents]
     ...
     * [StateFlow.value] observe the same value by construction (FR-008).
     *
     ...
     *
     * Cannot prevent the capture that triggered it (FR-013) — use [Capability.ScreenshotPrevention]
     * for that. Carries no payload, since any payload would risk conveying the very content the
     * library exists to protect (FR-025).
     *
     * An empty stream where unsupported, never an error (FR-014). Check
     * [supportLevel] for [Capability.ScreenshotEvents] to distinguish "no screenshots taken" from
     * "this platform cannot tell you".
     */
    public val screenshotEvents: Flow<Unit> get() = guardCore.screenshotEvents

    /**
     * Emits when a prevention mechanism fails to install or stops working (FR-022c).
     *
     ...
     * which is what makes [AppSwitcherProtection.Always] usable on its own (FR-015c).
     */
     ...
     *
     * Consent is **per capability** (FR-023a). There is no switch that enables all of them, and
     * consent granted here never extends to capabilities added in a later version — a library
     * upgrade cannot broaden what an application agreed to.
     *
     * [failurePosture] is required at this same call (FR-023b), not defaulted, because a mechanism
     * that can vanish mid-session must have an answer to "what happens to the content then?" before
     * it is switched on. See [FailurePosture].
     *
     * A no-op, not an error, when [capability] needs no opt-in, when [acknowledgement] names a
     * different capability, or when it does not accept the risk (FR-021).
     *
    ...
    /**
     * Which unsanctioned capabilities have been opted into (FR-023c).
     *
     ...
     * apply them to are held, never dropped (the spec's "no host available" edge case).
     */
 ...
 * argument is a compile-time constant reference instead of a fresh allocation on every
 * recomposition (constitution Principle V, SC-006).
 */
