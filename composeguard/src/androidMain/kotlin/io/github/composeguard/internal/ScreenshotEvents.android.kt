 ...
 * `ContentObserver` on MediaStore, which needs `READ_MEDIA_IMAGES` — a permission FR-026 forbids
 * the library from requesting — and is heuristic across OEMs anyway (research.md R6).
 *
...
internal class ScreenshotEvents {
    ...
    fun events(): Flow<Unit> {
        ...
        return callbackFlow {
            ...
            if (activity == null) {
                // Empty rather than an error: FR-014 requires an unsupported or unavailable stream
                // to be silent, so a consumer's collector is never handed an exception.
                awaitClose { }
                return@callbackFlow
            }
